...
export const remoteCacheGcsRunner: ReturnType<typeof createCustomRunner> =
  createCustomRunner<RemoteCacheGcsOptions>(
    async (options): Promise<RemoteCacheImplementation> => {
      initEnv(options);

      const bucketName =
        process.env.NX_REMOTE_CACHE_BUCKET ?? options.bucketName;
      ...
      const gcsRemoteCache = new GCSRemoteCache(
      ...
      return {
        ...
        storeFile: (fileName, stream) =>
          gcsRemoteCache.upload(fileName, stream),
      };
    }
  );
